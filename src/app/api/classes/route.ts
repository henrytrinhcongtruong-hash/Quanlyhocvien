...
export async function DELETE(req: NextRequest) {
  try {
    ...
    // 2. Xóa cascade toàn bộ dữ liệu liên quan (Thu quỹ, Điểm danh, Lịch trực, Sự kiện, Lịch thi, Thời khóa biểu, Sơ đồ lớp, Học sinh)
    if (studentIds.length > 0) {
      await prisma.$transaction([
        ...
        prisma.seatingChart.deleteMany({ where: { lop } }),
